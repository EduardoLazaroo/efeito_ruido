...
def plot_synthetic_panel(images):
    ...
    for idx, (img, ax) in enumerate(zip(images, axes)):
        ax.imshow(img)
        ax.axis('off')
        ax.set_title(f'Imagem {idx}', fontsize=12, weight='bold')
        ax.text(0.5, -0.12, 'Sintética', transform=ax.transAxes, ha='center', fontsize=10)
    fig.suptitle('Painel de Imagens Sintéticas Originais', fontsize=16, weight='bold')
    return save_fig(fig, SYNTH_DIR / 'synthetic_gallery')


def plot_noise_panel(image, image_idx, noise_images, noise_params):
    ...
    fig.suptitle(f'Aplicação de Ruídos na Imagem Sintética {image_idx}', fontsize=16, weight='bold')
    return save_fig(fig, NOISE_DIR / f'noise_gallery_image_{image_idx}')


def plot_gradcam_comparison(image_idx, image, original_cam, noisy_image, noisy_cam, metrics, noise_type, level):
    ...
    fig.suptitle(f'Comparação Grad-CAM - Imagem {image_idx} / {NOISE_CONFIGS[noise_type]["label"]} / {LEVEL_LABELS[level]}', fontsize=16, weight='bold')
    path_root = GRADCAM_DIR / f'gradcam_image_{image_idx}_{noise_type}_{level}'
...
def plot_publication_figure(images, df):
    image = images[0]
    rng = np.random.default_rng(SEED)
    noise_variant = {}
    cams = {}
    ...
    fig.suptitle('Figura de Publicação: Robustez Grad-CAM sob Ruídos', fontsize=18, weight='bold')
    return save_fig(fig, PUB_DIR / 'publication_main_figure')


def main(num_images=5):
    create_dirs()
    df = load_metrics()
    images = ensure_synthetic_images(num_images=num_images)
    plot_synthetic_panel(images)
    ...
    if df.empty:
        df = pd.DataFrame(columns=['image_idx', 'noise_type', 'noise_level', 'cosine_similarity', 'iou', 'entropy_original', 'entropy_perturbed'])

    for image_idx, image in enumerate(images[:num_images]):
        noise_images = {'gaussian': {'original': image}, 'salt_pepper': {'original': image}, 'speckle': {'original': image}}
        current_metrics = []
        for noise_type in NOISE_CONFIGS:
            for level in LEVEL_ORDER:
                noisy_image = get_noise_image(image, noise_type, level, rng)
                noise_images[noise_type][level] = noisy_image
                img_tensor = image_to_tensor(image, device)
                noisy_tensor = image_to_tensor(noisy_image, device)
                original_cam = compute_cam(model, img_tensor, target_layer)
                noisy_cam = compute_cam(model, noisy_tensor, target_layer)
                original_cam = cv2.resize(original_cam, IMAGE_SIZE)
                noisy_cam = cv2.resize(noisy_cam, IMAGE_SIZE)
                metrics = {
                    ...
                    'noise_level': level,
                    'cosine_similarity': cosine_similarity(original_cam, noisy_cam),
                    'iou': intersection_over_union(original_cam, noisy_cam),
                    'entropy_original': spatial_entropy(original_cam),
                    'entropy_perturbed': spatial_entropy(noisy_cam),
                }
                current_metrics.append(metrics)
                plot_gradcam_comparison(image_idx, image, original_cam, noisy_image, noisy_cam, metrics, noise_type, level)
        plot_noise_panel(image, image_idx, noise_images, NOISE_CONFIGS)
        df = pd.concat([df, pd.DataFrame(current_metrics)], ignore_index=True)

    df.to_csv(VIS_DIR / 'metrics' / 'visualization_metrics.csv', index=False)
    print(f'✓ Métricas de visualização salvas em: {VIS_DIR / "metrics" / "visualization_metrics.csv"}')
    stats_paths = plot_global_statistics(df)
    if stats_paths:
        for p in stats_paths:
            print(f'✓ Estatística salva em: {p}')
    plot_publication_figure(images[:num_images], df)
    print('✓ Visualizações concluídas em:', VIS_DIR)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Gerar visualizações científicas de Grad-CAM e ruído.')
    parser.add_argument('--max-images', type=int, default=5, help='Número de imagens sintéticas a processar')
    args = parser.parse_args()
    main(num_images=args.max_images)
